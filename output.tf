output "mssql_fqdn" {
  ...
  value = azurerm_sql_server.testmssql.fully_qualified_domain_name
}

output "demo_instance_ip" {
  description = "The actual ip address allocated for the resource."
  value       = azurerm_network_interface.testmssql-instance.private_ip_address
}

output "demo_instance_public_ip" {
  description = "The actual public ip address allocated for the resource."
  value       = azurerm_public_ip.testmssql-instance.ip_address
}
...
output "mssql_connect_string" {
  ...
  value       =  "Server=tcp:${azurerm_sql_server.testmssql.fully_qualified_domain_name} Database=${azurerm_sql_database.training.name};User ID=${azurerm_sql_server.testmssql.administrator_login};Password=${azurerm_sql_server.testmssql.administrator_login_password};Trusted_Connection=False;Encrypt=True;"
}
